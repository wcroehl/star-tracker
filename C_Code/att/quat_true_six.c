...
{
   FILE   *quat50, *quat10, *delta, *bigD ;   
   int    i, opp_sign ;     
   char   cat[16], cstk[4], ca[4][17], cqt[16], cqh[5], cq[4][17],
          fn_dir1[FNLEN], fn_dir2[FNLEN], fn_dir3[FNLEN], fn_tmp[FNLEN] ;
   double t_a, t_q ;
   ...
   quat10 = fopen(fn_tmp,"r") ;

   strcpy(fn_tmp, fn_dir2) ; strcat(fn_tmp, "/ckg_1b.dat") ;
   quat50   = fopen(fn_tmp,"r") ;

   strcpy(fn_tmp, fn_dir2) ; strcat(fn_tmp, "/diff_true_quat.dat") ;
   delta = fopen(fn_tmp,"w") ;

   strcpy(fn_tmp, fn_dir2) ; strcat(fn_tmp, "/diffB_true_quat.dat") ;
   bigD = fopen(fn_tmp,"w") ;

   pi = 2.0*asin(1.0) ;

   fscanf(quat10,"%s %s %s %s %s %s", cat,cstk,ca[0],ca[1],ca[2],ca[3]) ;
   fscanf(quat50,"%s %s %s %s %s %s", cqt,cqh,cq[0],cq[1],cq[2],cq[3]) ;
   
   while ((atof(cat) - atof(cqt)) < 0.0)
      fscanf(quat10,"%s %s %s %s %s %s", cat, cstk, ca[0],ca[1],ca[2],ca[3]) ;

   fscanf(quat10," %s ",cat) ; 

   while(fscanf(quat50,"%s", cqt) != EOF) 
     {
     fscanf(quat50," %s %s %s %s %s",cqh, cq[0],cq[1],cq[2],cq[3]) ;
     while ( atof(cat) - atof(cqt) > 0.012) 
	 {
         if (fscanf(quat50,"%s",cqt) == EOF) exit(100) ;
         fscanf(quat50,"%s %s %s %s %s",cqh,cq[0],cq[1],cq[2],cq[3]) ;
	 }
     ...
     if( fabs(t_q - atof(cat)) < 0.001 ) 
         { 
         fscanf(quat10,"%s %s %s %s %s", cstk, ca[0],ca[1],ca[2],ca[3]) ;
         t_a = atof(cat) ; 
         ...
         if (fscanf(quat10," %s ", cat) == EOF) exit(100) ; 
         }
     }
