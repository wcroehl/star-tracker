...
main (int argc, char *argv[])
{
  FILE     *gyro, *sttr, *outmag, *outi, *outo, *outr, *outn,
           ...
           *outPT1, *outPT2,*outPT3, *outes1, *outes2, *out_ccdT,*outq0,*outnstar,
           *outcnt,*outccT,*outw,*outcrfC,*outids,*outidb,*outstarID_dm, *outMpC, 
           *OW_qtest, *OV_qtest, *On_qtest, *Ot_qtest; /*add four for qtest*/
  ...
  outes2 = fopen(fn_tmp, "w");
   ...
   /* Test OUTPUTS (2) */
   /*Get all input values for starID_dm*/
   strcpy(fn_tmp, fn_dir2); strcat(fn_tmp, "/outccdT.dat");
   out_ccdT = fopen(fn_tmp, "w"); /*ccd_time*/
   
   strcpy(fn_tmp, fn_dir2); strcat(fn_tmp, "/outq0.dat");
   outq0 = fopen(fn_tmp, "w");/*q0*/
  
   strcpy(fn_tmp, fn_dir2); strcat(fn_tmp, "/outnstar.dat");
   outnstar = fopen(fn_tmp, "w");/*nstar*/

   strcpy(fn_tmp, fn_dir2); strcat(fn_tmp, "/outcnt.dat");
   outcnt = fopen(fn_tmp, "w");/*cnt*/

   strcpy(fn_tmp, fn_dir2); strcat(fn_tmp, "/outccT.dat");
   outccT = fopen(fn_tmp, "w");/*cctime*/

   strcpy(fn_tmp, fn_dir2); strcat(fn_tmp, "/outw.dat");
   outw = fopen(fn_tmp, "w");/*w*/

   strcpy(fn_tmp, fn_dir2); strcat(fn_tmp, "/outcrfC.dat");
   outcrfC = fopen(fn_tmp, "w");/*crf_count*/

   strcpy(fn_tmp, fn_dir2); strcat(fn_tmp, "/outids.dat");
   outids = fopen(fn_tmp, "w");/*id_star*/

   strcpy(fn_tmp, fn_dir2); strcat(fn_tmp, "/outidb.dat");
   outidb = fopen(fn_tmp, "w");/*id_body*/

   
   strcpy(fn_tmp, fn_dir2); strcat(fn_tmp, "/outstarID_dm.dat");
   outstarID_dm = fopen(fn_tmp, "w");/*starID_dm input*/

   strcpy(fn_tmp, fn_dir2); strcat(fn_tmp, "/outMpC.dat");
   outMpC = fopen(fn_tmp, "w"); /*Mp for C, 09/12/23*/
 
   /*add for qtest*/ 
   strcpy(fn_tmp, fn_dir2); strcat(fn_tmp, "/OW_QTest.dat");
   OW_qtest = fopen(fn_tmp, "w");

   strcpy(fn_tmp, fn_dir2); strcat(fn_tmp, "/OV_QTest.dat");
   OV_qtest = fopen(fn_tmp, "w"); 

   strcpy(fn_tmp, fn_dir2); strcat(fn_tmp, "/On_QTest.dat");
   On_qtest = fopen(fn_tmp, "w");
   
   strcpy(fn_tmp, fn_dir2); strcat(fn_tmp, "/Ot_QTest.dat");
   Ot_qtest = fopen(fn_tmp, "w");
  /*****************************************************************/
  ...
  while( ccd_time <= TIMELIMIT )
    ...
    if ( ccd_time < cctime && nstar >= 3)  {
       ...
       rtn = 0 ;
       /*Add fprintf code*/
       fprintf(out_ccdT, "%15.6f \n",ccd_time);
       fprintf(outq0, "%15.6f %15.6f %15.6f %15.6f \n",q0[0],q0[1],q0[2],q0[3]);
       fprintf(outnstar, "%15.6f \n", nstar);
       fprintf(outcnt, "%15.6f \n", cnt);
       fprintf(outccT, "%15.6f \n", cctime);
       fprintf(outw, "%15.6f %15.6f %15.6f \n", w[0], w[1], w[2]);
       fprintf(outcrfC, "%15.6f \n", crf_count);
       fprintf(outids, "%15.6f \n", id_star);
       fprintf(outidb, "%15.6f \n", id_body);

       rtn = starID_dm(ccd_time, q0, nstar, &cnt,
                     cctime, w, crf_count, id_star, id_body,
                     outi, outr, outo, outmag, outdist, ixy, x, y, xmag, outMpC, 677, 700) ;/*add MpC, 09/12/23*/

       fprintf(outstarID_dm, "%15.6f %15.6f %15.6f\n", ccd_time,cnt,rtn);
    ...
    if (cnt >= 3)    /* QUEST calculation for QC */
        ...
        fprintf(Ot_qtest, "%15.6f \n", ccd_time); /*add for qtest*/

        rtn = q_method(ccd_time, qq, P_theta, cnt, &d_i, out1a, out2, outPT2, outPT3, OW_qtest, OV_qtest, On_qtest) ; /*added outPTs, 7/28/23*/
         if (rtn != 0)
            {
