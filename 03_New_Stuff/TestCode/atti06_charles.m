...
while( ccd_time <= TIMELIMIT ) 
    ...
    if ( ccd_time < cctime && nstar >= 3)
        ...
        [ccd_time, cnt ,m_star, BLI, NEWBLI, rtn, b_star, crf, nstar] = starID_pm(ccd_time, q0, nstar, cnt, bore_body, outi, outr, outo, outmag, ixy, x, y, xmag, adjcell, scell, stars, m_star, b_star_prev, crf_prev,rtn_prev);        
        b_star_prev = b_star;
        crf_prev = crf;
        rtn_prev = rtn;
        fprintf("NEWBLI\n") ;        %  !!PRINT STATEMENT!!<-----------------------------------------------------------------*/
    ...
    if (cnt >= 3)
        ...
        [qq, P, d_i, rtn] = q_method(V, W, nstar, d_i);
...
% 		  cuvfout = cuvf(ccd_time, t_old, t_elapse, qq, q, P, w, b_est, cnt, zsum, znum, ccdstep); I dont like this
            [zsum, znum, b_est, q, t_old, P] = cuvf(ccd_time, t_old, t_elapse, qq, q, P, w, b_est, cnt, zsum, znum, ccdstep);
          cuvfout = 0;
		  b_est_average = b_est;
...
% 		  rg = no_stars(q0, w) ;  no idea
		  ...
		  fprintf(out1b,"  %15.12f",qp) ;
		  fprintf(out1b,"\n") ;
		  %fprintf(qc,"%15.6f  %3d %6.2f\n",ccd_time, QC1, atti_rms) ; dont have
	  ...
	  while nstar == 0
		  ...
		  rg = no_stars(ccd_time, t_gyro, q0, w) ;    dont have
		  qp = q0;
