function [count, candi_num, rtn] = new_obs(t, t_start, t_end, count, candi_num, oldnum,...
...
if (N_zone ~= previ_N_zone)
    cell = scell2(N_zone).num_stars;
    for j=1:1:cell
        if (scell2(N_zone).star_num(j) == oldnum)%change 11/17
            thisnum = j;
...
if (thisnum > scell2(N_zone).num_stars) 
    thisnum = scell2(N_zone).num_stars ;  
    if (t > t_start && t < t_end) 
        %fprintf(stdout, "Here2? thisnum=%4d\n", thisnum) ;
        fprintf("Here2? thisnum=%4d\n", thisnum) ;
    end
end
...
while(((abs(ra_diff_BD) < BD_limit*3600+1800.) || (abs(ra_diff_new) < FOV_limit*3600+1800.)) ...
    ...
    if (t > t_start && t < t_end)
        ...
        fprintf("+++ ra_diff_BD = %9.2f, ra_diff_new = %9.2f\n", ...
            ra_diff_BD, ra_diff_new);
...
while(((abs(ra_diff_BD) < BD_limit*3600+1800.) || (abs(ra_diff_new) < FOV_limit*3600+1800.)) ...
    ...
    i_ra  = stars(scell2(N_zone).star_num(thisnum-i)-1).ra  ;
    i_dec = stars(scell2(N_zone).star_num(thisnum-i)-1).dec ;
    i_mag = stars(scell2(N_zone).star_num(thisnum-i)-1).mag ;
...
while (((abs(ra_diff_BD) < BD_limit*3600+1800.) || (abs(ra_diff_new) < FOV_limit*3600+1800.)) ...
    ...
    if (t > t_start && t < t_end)
        ...
        %    stars(scell2(N_zone).star_num(thisnum+i)-1).cat_num, i) ;
        fprintf("2ND: t=%15.6f N_zone=%3d thisnum=%3d\n", t, N_zone, thisnum);
        fprintf("i_ra = %9.2f, i_dec = %9.2f, i_mag = %6.2f\n", ...
            i_ra, i_dec, i_mag) ;
        fprintf("    star_num = %5d (%2d)\n", ...
            stars(scell2(N_zone).star_num(thisnum+i)).cat_num, i) ;
    end
    ...
    if(abs(arc_dist) < vici && abs(i_mag - new_mag) < mTOL) %/* a possible match */
        thisnum1 = thisnum+i+1 ;
        for j=1:1:id_count+1 % add 1, 1/9/24
        %for j=0:1:id_count 
            if(id_star(j) == stars(scell2(N_zone).star_num(thisnum+i)-1).cat_num) 
                astaragain = 1 ;
            end
    ...
    i = i + 1;
    astaragain = 0 ;
    if( (thisnum + i - 1) >= scell2(N_zone).num_stars) %/* break ; */
        thisnum = thisnum - scell2(N_zone).num_stars ;  
        if (t > t_start && t < t_end) 
